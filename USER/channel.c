...
{
	"ADC1",	"ADC2",	"ADC3",	"ADC4",	" S1 ",	" S2 ",	" SA ",	" SB ",	" SC ",	" SD ",
	"PPM1",	"PPM2",	"PPM3",	"PPM4",	"PPM5",	"PPM6",	"PPM7",	"PPM8",
	"SBS1",	"SBS2",	"SBS3",	"SBS4",	"SBS5",	"SBS6",	"SBS7",	"SBS8",
	"SBS9",	"SB10",	"SB11",	"SB12",	"SB13",	"SB14",	"SB15",	"SB16",
...
void input_setup()
{
	...
	{
		first_splash=0;
		if(channel_count>channel_y_count)menu_len=channel_y_count;
		else menu_len=channel_count;	
		px=py=0;
		menu_y=0;
	}
	...
	if(LEFT)
	{
		LEFT=0;
		if(px)px--;
		else px=channel_x_count-1;
	}
	if(RIGHT)
	{
		RIGHT=0;
		if(px<(channel_x_count-1))px++;
		else px=0;
	}
	...
	if(param_edit)
	{
		...
		if(UP)
		{
			...
			{
				case 1:	
					if(ch_map[py]<(ch_input_count-1))ch_map[py]++;
					else ch_map[py]=0;
					break;
				case 2:
					if(ch_min[py]<250)ch_min[py]+=UP;
					if(ch_min[py]>250)ch_min[py]=250;
					break;					
				case 3:
					if(ch_mid[py]<250)ch_mid[py]+=UP;
					if(ch_mid[py]>250)ch_mid[py]=250;
					break;	
				case 4:
					if(ch_max[py]<250)ch_max[py]+=UP;
					if(ch_max[py]>250)ch_max[py]=250;	
					break;
			}
			UP=0;
		}
		...
		if(DOWN)
		{
			...
			{
				case 1:	
					if(ch_map[py])ch_map[py]--;
					else ch_map[py]=ch_input_count-1;
					break;
				case 2:
					if(ch_min[py]>-250)ch_min[py]-=DOWN;
					if(ch_min[py]<-250)ch_min[py]=-250;
					break;					
				case 3:
					if(ch_mid[py]>-250)ch_mid[py]-=DOWN;
					if(ch_mid[py]<-250)ch_mid[py]=-250;
					break;	
				case 4:
					if(ch_max[py]>-250)ch_max[py]-=DOWN;
					if(ch_max[py]<-250)ch_max[py]=-250;	
					break;
			}
			DOWN=0;
	...
	else
	{
		if(UP)
		{
			...
			{
				py=channel_count-1;
				if(channel_count>channel_y_count)menu_y=channel_count-channel_y_count;
				else menu_y=0;
			}	
			if(py<menu_y)menu_y--;
		}
		if(DOWN)
		{
			...
			else if(py>=(menu_y+channel_y_count))menu_y++;
		}
		if(ESC)
		{
			ESC=0;
